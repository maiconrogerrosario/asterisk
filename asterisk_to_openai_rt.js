...
function startExternalWebSocket(channelId) {
  logger.info(`Attempting to start External WebSocket for channel ${channelId}`);

  const ws = new WebSocket(process.env.EXTERNAL_WS_URL);
  ...
  const initializeStreamHandler = async () => {
    ...
    if (channelData && channelData.rtpSource) {
      ...
      logger.info(`StreamHandler initialized for channel ${channelId}`);
    }
    return streamHandler;
  };

  ws.on('open', async () => {
    logger.info(`[Client] External WebSocket connected for channel ${channelId}`);
    await initializeStreamHandler();

    maxCallTimeoutId = setTimeout(async () => {
      logger.info(`Max call duration reached for channel ${channelId}`);
      ws.close();
      await ariClient.channels.hangup({ channelId });
    }, MAX_CALL_DURATION);
  });

  // 🔹 RECEBE áudio do outro serviço → Asterisk
  ws.on('message', async (data) => {
    if (!Buffer.isBuffer(data)) return;

    if (streamHandler) {
      streamHandler.write(data); // PCM linear direto
    }
  });

  ws.on('error', (err) => {
    logger.error(`External WebSocket error for channel ${channelId}: ${err.message}`);
  });

  ws.on('close', () => {
    if (maxCallTimeoutId) clearTimeout(maxCallTimeoutId);
    if (streamHandler) streamHandler.stop();
    logger.info(`[Client] External WebSocket closed for channel ${channelId}`);
  });
  ...
  return {
    ...
    sendAudio: (pcmBuffer) => {
      if (ws.readyState === WebSocket.OPEN) {
        ws.send(pcmBuffer); // payload-only
      }
    },
    stopStream: () => streamHandler && streamHandler.stop(),
    getPlaybackComplete: () => true
  };
}
