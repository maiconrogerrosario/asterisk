...
function detectAtendente(text) {
  if (!text) return false;

  const regex = /\batendente\b/i;
  return regex.test(text);
}


async function startOpenAIWebSocket(channelId,onTransfer) {
      ...
      switch (response.type) {
        ...
        case 'conversation.item.input_audio_transcription.completed':
          if (response.transcript) {
            ...
            logOpenAI(`🧑 Usuário disse: ${text}`, 'info');

            if (detectAtendente(text)) {
              logger.info(`📞 Usuário pediu atendente → transferindo`);

              await waitForBufferEmpty(channelId);

              if (onTransfer) {
                await onTransfer("PJSIP/1001");
              }
  ...
  const connectWebSocket = () => {
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', async () => {
        ...
        try {
          const rtpSource = channelData.rtpSource || { address: '172.31.0.47', port: 12000 };
