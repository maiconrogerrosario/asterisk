...
async function initializeAriClient() {
    ...
    // Conecta no ARI (Asterisk REST Interface)
    ariClient = await ari.connect(config.ARI_URL, config.ARI_USER, config.ARI_PASS);

    // Inicia a aplicação ARI
    await ariClient.start(config.ARI_APP);

    ariClient.on('StasisStart', function (event, channel) {});
    ariClient.channels.originate(
      {endpoint: 'PJSIP/1001', app: 'application', appArgs: 'dialed'},
